...
uint32 poll_tx_ts, resp_rx_ts, trep2, trep1 , poll_rx_ts, resp_tx_ts ,  Tround1 ,  Tround2 ,Treply1 , Treply2;
...
int ss_resp_run(void)
{
  ...
  if (status_reg & SYS_STATUS_RXFCG)
  {
    ...
    * As the sequence number field of the frame is not relevant, it is cleared to simplify the validation of the frame. */
    ...
    if (memcmp(rx_buffer, tx_poll_msg, ALL_MSG_COMMON_LEN) == 0)
    {
      uint32 resp_tx_time;
      int ret;
     ...
     // resp_msg_set_ts(&tx_resp_msg[RESP_MSG_POLL_RX_TS_IDX], );
      ...
      resp_msg_set_ts(&rx_resp_msg[RESP_MSG_POLL_RX_TS_IDX], poll_rx_ts);
      resp_msg_set_ts(&rx_resp_msg[RESP_MSG_RESP_TX_TS_IDX], resp_tx_ts);

      Treply1 = resp_rx_ts - poll_rx_ts;

      /* Write and send the response message. See NOTE 9 below. */
      rx_resp_msg[ALL_MSG_SN_IDX] = frame_seq_nb;
      dwt_writetxdata(sizeof(rx_resp_msg), rx_resp_msg, 0); /* Zero offset in TX buffer. See Note 5 below.*/
  ...
  if (status_reg & SYS_STATUS_RXFCG)
  {
    ...
    * As the sequence number field of the frame is not relevant, it is cleared to simplify the validation of the frame. */
    ...
    if (memcmp(rx_buffer, tx_poll_msg, ALL_MSG_COMMON_LEN) == 0)
    {
      ...
      resp_msg_get_ts(&rx_buffer[RESP_MSG_RESP_TX_TS_IDX], &Treply1);
