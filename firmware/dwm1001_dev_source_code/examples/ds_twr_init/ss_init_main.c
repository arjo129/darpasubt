...
static volatile int firstTxCount = 0 ; // Successful transmit counter
static volatile int secondTxCount = 0;
...
int ss_init_run(void)
{
  ...
  initiatorMessage[ALL_MSG_SN_IDX] = frame_seq_nb;
  dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_TXFRS);
  dwt_writetxdata(sizeof(initiatorMessage), initiatorMessage, 0); /* Zero offset in TX buffer. */
  dwt_writetxfctrl(sizeof(initiatorMessage), 0, 1); /* Zero offset in TX buffer, ranging. */

  /* Start transmission, indicating that a response is expected so that reception is enabled automatically after the frame is sent and the delay
  * set by dwt_setrxaftertxdelay() has elapsed. */
  dwt_starttx(DWT_START_TX_IMMEDIATE | DWT_RESPONSE_EXPECTED);
  firstTxCount++;
  printf("Initiator - 1st Transmission # : %d\r\n", firstTxCount);
  ...
  if (status_reg & SYS_STATUS_RXFCG) {		
    ...
    * As the sequence number field of the frame is not relevant, it is cleared to simplify the validation of the frame. */
    ...
    if (memcmp(rx_buffer, responderMessage, ALL_MSG_COMMON_LEN) == 0) {	
      ...
      // resp_msg_get_ts(&rx_buffer[RESP_TX_1_TS_IDX], &respTxTimestamp1);

      /* Compute final message transmission time. */
      initTxDelay = (initRxTimestamp1 + (POLL_RX_TO_RESP_TX_DLY_UUS * UUS_TO_DWT_TIME)) >> 8;
      dwt_setdelayedtrxtime(initTxDelay);

      /* Response TX timestamp is the transmission time we programmed plus the antenna delay. */
      initTxTimestamp2 = (((uint64)(initTxDelay & 0xFFFFFFFEUL)) << 8) + TX_ANT_DLY;

      /*n Write all time stamps into the third, final message */
      // resp_msg_set_ts(&finalMessage[INIT_TX_1_TS_IDX], initTxTimestamp1);
      // resp_msg_set_ts(&finalMessage[INIT_RX_1_TS_IDX], initRxTimestamp1);
      // resp_msg_set_ts(&finalMessage[INIT_TX_2_TS_IDX], initTxTimestamp2);

      resp_msg_set_ts(&initiatorMessage[INIT_TX_1_TS_IDX], initTxTimestamp1);
      resp_msg_set_ts(&initiatorMessage[INIT_RX_1_TS_IDX], initRxTimestamp1);
      resp_msg_set_ts(&initiatorMessage[INIT_TX_2_TS_IDX], initTxTimestamp2);

      /* Write and send the third message */
      initiatorMessage[ALL_MSG_SN_IDX] = frame_seq_nb;
      dwt_writetxdata(sizeof(initiatorMessage), initiatorMessage, 0);
      dwt_writetxfctrl(sizeof(initiatorMessage), 0, 1);
      /* Start a delayed transmission. Response NOT expected since this is the final message to be sent out. */
      ...
      if (ret == DWT_SUCCESS) {
        ...
        secondTxCount++;
        printf("Initiator - 2nd Transmission # : %d\r\n", secondTxCount);
