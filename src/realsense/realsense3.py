...
def plot3d(pA ,pB):
    ...
    plt.savefig("test"+str(rand.randint(1,1000))+".png")
...
try:
    while True:
        ...
        # canny = cv2.Canny(depth_image, 100, 200)
        # depth_image = cv2.adaptiveThreshold(canny, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
        ...
        if (first):
            ...
            if p0 is not None:
                ...
                for i in range(NUMofP):
                    depth_point = getnewAvgDepth(aligned_depth_frame,MatrixInit[i] ,depth_intrin )
            ...
            #frame_gray = cv2.erode(frame_gray, kernel, iterations=1)
            ...
            if p1 is not None:
                ...
                for i in range(NUMofP):
                    depth_point = getnewAvgDepth(aligned_depth_frame, MatrixPts[i], depth_intrin , MatrixInit[i])
                ...
                # Matrixdiff = MatrixPts -MatrixInit
                #
                # velocity = []
                #
                # for x in Matrixdiff:
                #     magni = math.sqrt(sum(i ** 2 for i in x))
                #     velocity.append(magni)
                # # print(MatrixInit)
                #
                # # print(velocity)
                ...
                # if(len(MatrixInit) !=  0 and len(MatrixPts) != 0):
                #     x,y,z = RT.getMovement(MatrixInit , MatrixPts)
                #
                #     X += x
                #     Y += y
                #     Z += z
                ...
                try:
                    ...
                    # RT.getAngleZ(MatrixInit, MatrixPts)
                    # R, T = rot.rigid_transform_3D(MatrixInit , MatrixPts)
                    # print("VALUES:")
                    # print(R)
                    # print(T)

                    # degree = (RT.rotationMatrixToEulerAngles(R))
                    ...
                    # degree = RT.getkabschangle(MatrixInit , MatrixPts)
                    if (abs(R[0][0]) < 1):
                    ...
                    Deg_str = "Degree: %f" % DEG
                    rospy.loginfo(Deg_str)
                    pub.publish(Deg_str)
                    # # print("degree")
                    # print(degree)
                    print(DEG)
